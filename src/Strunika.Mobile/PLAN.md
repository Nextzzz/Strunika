...
- **Плавність**: позиція **прогнозується** з кадрового тікера платформи (`Animation.Commit`, vsync) і лише кожні
  200 мс звіряється з транспортом (`SongViewModel.Frame`). Фризи ~100 мс щосекунди на Windows виявились
  індукованими повними збираннями GC від WinUI (нативні об'єкти Win2D/слайдера щокадру) — тому стрічка конвеєра
  малюється у широкі канваси раз на кілька секунд і щокадру лише **зсувається трансформацією** (два кольори через
  обрізані контейнери, подвійний буфер), слайдери замінено на `SeekBar`, перетягування — через `PointerDrag`
  (нативний захват вказівника на Windows). Правило — skill §7.
