...
**Editor round (2026-09-14).** The editor has **two states and no third**: a chord is chosen, or the panel says «Обери акорд на дошці». Nothing is ever chosen for the reader — not on the way in, not after an undo (it restores the choice as it was), not after a delete. The last choice in each song is remembered by where the chord begins for as long as the app runs, and comes back with the editor. In the beat view a square holds a chord only if it is the **beat nearest that chord's start** (`Models/BeatMath.Nearest`) — the one rule the grid draws by and the editor chooses by; an empty square lets go of the chord and is where a new one goes. On the track chords **snap to the sixteenth** (a beat cut in four, no buzz), the arrows step a sixteenth and a chord is added under the cursor to the sixteenth; in the beat view the arrows step a square. The chord being worked on wears **dotted handles** (bars read as "stretch me"). Taking hold of one buzzes, leaves its old place empty (covered in the page's ground on the track; its square repainted without it in the grid) and lifts it onto the finger in the lifted colours — text colour for ground, the page's own for ink, an accent ring — and it stays lifted where it lands until the song's chords come back with the move. **Played chords take their own colour in both views** while editing; on the track the played/coming halves meet at the real playhead, moved by transforms. The map draws the window exactly as the track sees it (no least width, never pushed back inside; the grips sit outside it), `ChordTrack.LookAt` centres what it is given, and on iOS a drag on the map starts where the finger came down (`Platforms/iOS/TouchDownRecognizer`, fed into `PointerDrag.Started`). The follow chip is its icon alone, and the way back to the chosen chord wears the same icon; their row keeps its height, so they never take room from the track. For a YouTube song the editor's level slider is the device volume, like the "…" sheet's. The chosen chord's name in the panel is measured and shrunk to its chip, never cut.
