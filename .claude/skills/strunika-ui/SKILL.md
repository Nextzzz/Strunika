...
### The tuner's headstock (design, 2026-09-11)
The pegs sit as on a 3+3 headstock: **half the strings on the left, half on the right, in columns that lean in towards the top**, the outer strings' pegs nearest the nut (low E bottom-left, high e bottom-right), so the strings fan out without crossing. **A string runs from every peg down under the tab bar**, where it fades, in its peg's colours: idle `Separator` (the idle peg's ring is `Separator` too), being tuned `Accent`, tuned `Fill`; the bass strings are thicker. **The string being tuned shakes while it sounds** — wider the harder it is played, dying with the note, beating (swelling and fading) while out of tune and steady once in; a string that gets tuned twangs, and the celebration twangs them all in a wave. The geometry comes from the measured peg size, the height from the page (`TunerView.ArrangeNeck`: a share of the free height between what the pegs need and what they can use) and never from the "start again" chip, so no peg moves under a finger. Per-frame rule: the resting strings are one canvas repainted on colour changes; a sounding string is a bowed copy drawn once and swung with `NativeTransform.ScaleXAround`.
