...
### The chord editor (design, 2026-09-09)
The editor is a **way of playing the song, not another page**: the YouTube player, the transport, the play button and the chord steps stay exactly where they are, and it works in both views of the song. What it adds is a row of its own over the play button — move ◀ ▶ · the chord being worked on · add + · delete 🗑 — and what pays for that row is the **chord diagrams, which give up their row in the conveyor view**; the beat view simply shows fewer squares. The **YouTube player is folded away and cannot be unfolded** while it is on. The A–B chip's place becomes **Готово**, the way out; the "…" sheet stays, so the view switch is still a tap away.
**The chord under the playhead is the one being worked on** — no other selection exists, so the steps, a tap on a pill and a tap on a square all choose it. The editor is behind Pro, so it shows the song **exactly as stored: neither simplified nor transposed**, and the picker offers the whole vocabulary (root first, then quality). Replacing a chord asks, before the choice, whether every chord of that name in the song goes with it. Every change is saved as it is made — there is no "save" and nothing to lose by leaving.
